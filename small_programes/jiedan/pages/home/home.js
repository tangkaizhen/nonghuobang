...
Page({
  ...
  order_type_word_to_num(){
    ...
    if (order_type == '' || order_type == '全部订单') {
      order_type_num = 0
    } else if (order_type == '申请中') {
      order_type_num = 1
    } else if (order_type == '待确认') {
      order_type_num = 2
  ...
  order_list(e){
    ...
    _this.setData({
      order_type_select: {
        ...
        order_type1: (order_index == 1 ? true : false),
        order_type2: (order_index == 2 ? true : false),
  ...
  order_assign_box(){
    ...
    _this.setData({
      order_assign_select_img: !_this.data.order_assign_select_img
    })
  },
  order_btn_cancel(){
    this.filtrate_reset()
  ...
  order_btn_confirm(){
    ...
    _this.setData({
      ...
      isAssigned: _this.data.order_assign_select_img,
      orders:[]
    })
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          ...
          wx.request({
            url: app.globalData.api_ctx + '/helper/rejectOrder.json', // 仅为示例，并非真实的接口地址
            data: {
              id: wx.getStorageSync('helper_id'),
  ...
  onPullDownRefresh: function () {
    ...
    wx.request({
      ...
      data: {
        ...
        isAssigned: _this.data.isAssigned,
        pageNum: 1,
        appToken: wx.getStorageSync('appToken')
      },
      header: wx.getStorageSync('header'),
